...
class Forward:
    @free_fall_migration(document_models=[BMIRecordBaseDocument])
    async def upgrade(self, session):
        collection_name: str = ""
        try:
            collection_name = BMIRecordBaseDocument.Settings.name
            logger.info(
                f"Starting checking collection '{collection_name}' existence..."
            )
            async_collection = BMIRecordBaseDocument.get_pymongo_collection()
